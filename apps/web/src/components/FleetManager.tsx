...
export const FleetManager: React.FC = () => {
  ...
  return (
    <div className="product-page fleet-page">
      ...
      {tab === 'all' && (
        <>
          <section className="fleet-summary"><article><Cpu size={16} /><div><strong>{models.length}</strong><span>{isArabic ? 'موديل مسجل' : 'registered models'}</span></div></article><article><Bot size={16} /><div><strong>{agents.filter((item) => item.execution_ready).length}</strong><span>{isArabic ? 'وكيل جاهز للتنفيذ' : 'execution-ready agents'}</span></div></article><article><Cloud size={16} /><div><strong>{connectedProviderCount}</strong><span>{isArabic ? 'مزود متصل' : 'connected providers'}</span></div></article><article><Sparkles size={16} /><div><strong>{skills.length}</strong><span>{isArabic ? 'مهارة مسجلة' : 'registered skills'}</span></div></article></section>
